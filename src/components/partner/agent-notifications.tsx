...
import { Bell, CalendarClock, ClipboardCheck, CreditCard, Inbox, KeyRound, MessageSquare, ShieldCheck, UserCog, Wrench } from "lucide-react";
...
import {
  ...
  type ProfessionalNotificationCategory,
} from "@/lib/professional-work";

// Agent Dashboard Redesign phase -- Notifications, previously a
// placeholder, now a real feed (Section 44/45), reusing the Owner
// notification page's exact design pattern (grouped Today/Yesterday/
// Earlier, category icons, mark-all-read) rather than inventing a new one.
//
// Property Manager Dashboard phase -- generalized to accept a role, so the
// exact same feed/behavior serves both AgentNotifications (unchanged
// export, unchanged behavior) and the new PmNotifications, rather than
// forking a near-identical second component.

const CATEGORY_ICONS: Record<ProfessionalNotificationCategory, typeof Bell> = {
  enquiry: Inbox,
...
export function AgentNotifications() {
  return <ProfessionalNotifications role="agent" />;
}

export function PmNotifications() {
  return <ProfessionalNotifications role="property_manager" />;
}

function ProfessionalNotifications({ role }: { role: ProfessionalRole }) {
  const router = useRouter();
  const professional = useDemoProfessional(role);
  const professionalId = professional?.id ?? "";

  const getSnapshot = useCallback(() => getProfessionalNotifications(professionalId), [professionalId]);
  const getUnreadSnapshot = useCallback(() => getProfessionalUnreadCount(professionalId), [professionalId]);
  const notifications = useSyncExternalStore(subscribeToProfessionalWork, getSnapshot, getSnapshot);
  const unreadCount = useSyncExternalStore(subscribeToProfessionalWork, getUnreadSnapshot, getUnreadSnapshot);

  const handleClick = useCallback(
    (n: ProfessionalNotification) => {
      markProfessionalNotificationRead(n.id);
  ...
  return (
    <DashboardShell initialSection="notifications">
        ...
        <div className="mx-auto max-w-[760px]">
          <div className="flex flex-wrap items-start justify-between gap-4 border-b border-black/10 pb-8">
            <div>
              ...
              <p className="text-carbon-500 mt-2 text-sm">Enquiries, viewings, applications, and team updates across your properties.</p>
            </div>
            {unreadCount > 0 ? (
              <button type="button" onClick={() => markAllProfessionalNotificationsRead(professionalId)} className="text-carbon-500 text-xs font-medium underline underline-offset-4 hover:text-black">
                Mark all as read
              </button>
            ) : null}
          </div>
          ...
          <div className="mt-6">
            {grouped.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <p className="text-carbon-500 mt-3 max-w-xs text-sm leading-6">Important updates about your properties, enquiries and applications will appear here.</p>
              </div>
            )}
          </div>
        </div>
